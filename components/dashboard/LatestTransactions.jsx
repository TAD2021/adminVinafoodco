                    ...
                    {transactions.map((transaction, index) => (
                        <tr key={index} className="border-t border-gray-700">
                            <td className="py-2 flex items-center">
                                <img alt="User  profile" className="rounded-full w-8 h-8 mr-2" src="https://storage.googleapis.com/a1aa/image/eZwkVHxKW1y3NKXhvxeRcbCOdCwHTV6aThPbl0fik3MgIjWnA.jpg" />
                                {transaction.name}
                            </td>
                            <td className="py-2">
                                <span className={`bg-${transaction.status === "pending" ? "yellow" : transaction.status === "done" ? "green" : "red"}-500 text-black px-2 py-1 rounded`}>
                                    {transaction.status}
                                </span>
                            </td>
